...
- `engine/encounter`：**PvE 威胁结算原语**（设计 `docs/PvE威胁系统.md`）——`ContributionScore`（贡献评分，付费不进）、`AllocateLoot`（战利品按贡献分配：排他件走 `arbitration` 胜率∝Score/频率无关/付费不进，可分件 `SplitProportional`，败者补偿，反蹭场过滤）、`PenaltyCap/DegradePenalty`（失败惩罚后果分级闸，D0-D3 硬锁不可逆）、`SoloAllowed`（单人解锁门）。
