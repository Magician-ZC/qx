...
- `engine/decision` 的 `attribution.go`：**「意外但合理」的代码强制**（设计宪法 §5）。`ValidateAttribution(attr, snap)` 要求每个自治选择都带可解析的前因（人格/记忆/红线/关系/压力/回响），无源戏剧性意外判 OOC；`GateSurprise(action, in)` 把突然恋爱/卖传家宝/叛变硬绑前因。**已接入 `session.generateUnitDecision` 且线上默认开启强制**（`httpapi/router.go` 调 `SetAttributionEnforcement(true)`）：决策前 `prepareAttribution`（`session/attribution_bridge.go`）构造快照——人格(persona_trait)+压力(pressure)+实时记忆(memory，可引用 ID 暴露进 prompt)+对外关系四轴(relation，归一 [-1,1])，校验不过的归因优雅回退安全决策（继续待命）。遥测为**进程级全局**计数（`session.AttributionStats()`，每请求新建的 Service 不会重置），surfaced 在 `/healthz` 的 `attribution` 块（`total`/`ooc`/`ooc_rate`/`enforced`/`degraded`/`active`）。**自动降级护栏**：累计样本 ≥50 后 OOC 率 >15% 即 latch 降级（`AttributionDegraded()`），全局抑制强制转影子（`enforcementActive()=enforced && !degraded`），`/healthz` 标 `degraded:true`；`ResetAttributionDegrade()` 重新武装。`redline/order_echo` 待离线宪章/回响落地后接入。
- `engine/relevance`：**相关性与传播原语**（设计 `docs/事件耦合与跨玩家关联.md` 的「结算」核心）——`Score(hits, hopFidelity)` 用 **relative-importance 归一化 + noisy-OR** 组合（`1−Π(1−contribution)`，`contribution=weight·relativeImportance(kind)·timeDecay·hopFidelity`），单根强锚即可过 0.35 阈、多锚次可加、恒在 [0,1]、对 weight 单调（修了「加权和为 1 致单锚封顶过不了阈」的标定缺陷，有蒙特卡洛分布/单调测试）；另有 `HopFidelity(hop)=0.6^hop`、`StopPropagation`、`ConsentTierFor`（后果层→异步同意档）、`MatchScore`（撮合四因子）。
- **命运层（`session/fate.go`，设计宪法 §4 M1 + `docs/事件耦合与跨玩家关联.md`）**：`SurfaceFateEvent` 把世界事件按相关性（关系锚 / 自身事件按重要度）三档路由（自治/高光卡/待决策），待决策经 `events.EmitProcessEvent`（流程事件旁路，非状态变更）写入命运收件箱；`OpenFateInbox`/`ResolveFateDecision` 读/处理；`WorldizeDeath` 把一个角色之死路由进「在乎她的人」的收件箱——**已接入实战死亡路径**（`service.go` 战斗击杀后 best-effort 调用，绝不影响结算）。HTTP：`GET /api/fate/inbox/:unitId`、`POST /api/fate/decisions/:decisionId/resolve`。
- `engine/encounter`：**PvE 威胁结算原语**（设计 `docs/PvE威胁系统.md`）——`ContributionScore`（贡献评分，付费不进）、`AllocateLoot`（战利品按贡献分配：排他件走 `arbitration` 胜率∝Score/频率无关/付费不进，可分件 `SplitProportional`，败者补偿，反蹭场过滤）、`PenaltyCap/DegradePenalty`（失败惩罚后果分级闸，D0-D3 硬锁不可逆）、`SoloAllowed`（单人解锁门）。**已接入 session**：`session/threat.go` 的 `Service.ResolveEliteEncounter` 跑通单人 elite 全链路——撞见→反射护栏→`combat_roll` 多回合消耗战→胜利 `AllocateLoot` 分赃（钱包经 Mutator 留痕）/失败 `DegradePenalty` 惩罚（士气经 Mutator）→祖魂语气收件箱卡，有 DB 集成测试。尚未接入执行主循环（触发是后续：威胁刷新 + `decision.Router` 的 StrategicFork）。
