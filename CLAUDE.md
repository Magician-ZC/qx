...
- `engine/relevance`：**相关性与传播原语**（设计 `docs/事件耦合与跨玩家关联.md` 的「结算」核心）——`Score(hits, hopFidelity)`（锚命中→相关性，`weight·factorWeight·timeDecay·hopFidelity` 求和）、`HopFidelity(hop)=0.6^hop`、`StopPropagation`、`ConsentTierFor`（后果层→异步同意档）、`MatchScore`（撮合四因子）。注意 Relevance 是**加权求和**且因子权重和为 1，单根锚贡献以其权重为上限（重大 beat 需多锚共振）。
